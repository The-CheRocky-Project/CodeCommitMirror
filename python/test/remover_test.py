...
class TestRemover(unittest.TestCase):
    ...
    def test_file_is_deleted(self):
        """
        L'evento è scatenato dal caricamento di un
        file che quindi deve essere cancellato
        :return:
        """
        with mock_s3():
            ...
            # Dovrebbe lanciare un'eccezione siccome il file che
            # sto cercando di ottenere dovrebbe essere stato cancellato
            self.assertRaises(
                Exception,
    ...
    def test_remover_fail(self):
        """
        Durante l'esecuzione della lambda succede qualcosa di
        inaspettato (in questo test il bucket da cui cancellare
        non esiste) che deve far tornare False per avvisare che
