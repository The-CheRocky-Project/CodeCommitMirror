...
file_path2 = absolute_path + '/../event/change_label_status_uncheckRow_event.json'
file_path3 = absolute_path + '/../event/change_label_status_false_event.json'
...
class TestChangeLabelStatus(unittest.TestCase):
    ...
    @mock_s3
    def test_change_label_status_checkrow(self):
    ...
    @mock_s3
    def test_change_label_status_uncheckrow(self):
        with patch('boto3.client') as mock:
            s3 = boto3.resource('s3')
            s3.create_bucket(Bucket='ahlconsolebucket')
            resume = s3.Object('ahlconsolebucket', 'tmp/modified-resume.json')
    ...
    @mock_s3
    def test_change_label_status_false(self):
        s3 = boto3.resource('s3')
        s3.create_bucket(Bucket='ahlconsolebucket')
        resume = s3.Object('ahlconsolebucket', 'tmp/modified-resume.json')
        resumeJson = [
            ...
            {"frame_key": "frames/match.0000015.jpg", "accuracy": "0.969301", "label": "6", "start": "4750",
             "tfs": "8750", "type": "machine", "show": "true"}]
        resume.put(Body=json.dumps(resumeJson))
        result = lambda_handler(event_json3, CONTEXT)
        self.assertFalse(result)

    @mock_s3
    def test_change_label_status_exception(self):
        self.assertRaises(Exception, lambda_handler, event_json1, CONTEXT)
